...
class DefinitionMethod(private val method: Method) {
    ...
    init {
        ...
        classFactory = ClassFactory(method.containingClass ?: throw Exception("Failed to load ClassFactory from definition method"))
    }
        ...
        get() {
            ...
            method.childrenOfType<GroupStatement>().forEach { groupStatement ->
                groupStatement.childrenOfType<PhpReturn>().forEach { definitionReturn: PhpReturn ->
                    definitionReturn.childrenOfType<ArrayCreationExpression>().forEach { definition: ArrayCreationExpression ->
                        definition.childrenOfType<ArrayHashElement>().forEach { propertyDefinition: ArrayHashElement ->
